...
def send_to_satusehat(docname):
	...
	from clinic_satusehat.queue_core.doctype.satusehat_payload_generator.payload_builders.clinical_impression import ClinicalImpressionBuilder
	...
	builder = ClinicalImpressionBuilder()
	payload = builder.build(MockDoc())
	
	payload["identifier"][0]["value"] = docname
	payload["status"] = doc.clinical_status
	...
	try:
		...
		resp = requests.post(f"{base_url}/ClinicalImpression", json=payload, headers=headers, timeout=30)
		...
		if resp.status_code in [200, 201]:
			...
			frappe.db.commit()
			return {"status": resp.status_code, "message": "Berhasil mengirim ClinicalImpression."}
		else:
			return {"status": resp.status_code, "message": "Gagal mengirim ClinicalImpression. Periksa API Response."}
