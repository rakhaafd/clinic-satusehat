...
frappe.ui.form.on("DiagnosticReport SatuSehat", {
	refresh: function(frm) {
		if (frm.doc.status === "Valid" && !frm.doc.satusehat_id && !frm.is_new()) {
			frm.add_custom_button(__("Send to SatuSehat"), function() {
				frappe.call({
					method: "clinic_satusehat.diagnosticreport.doctype.diagnosticreport_satusehat.diagnosticreport_satusehat.send_to_satusehat",
					args: {
